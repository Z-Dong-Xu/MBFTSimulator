...
/**
 * @ClassName: MBFT_2_1_1_Replica
 * @Description:
 * @Author: Dongxu Zhu
 **/
public class MBFT_2_1_1_Replica extends Replica{
    ...
    public MBFT_2_1_1_Replica(int id, int[] netDlys, int[] netDlyToClis) {
        this.id = id;
...
//		2. 将本OV消息发送到主节点消息广播
    ...
    public void receiveOrderingV(Message msg){
        ...
        if(hasRequset(req_n)){
            ...
            if(ordered(req_n)){
                MessageDecisionMsg messageDecisionMsg = new MessageDecisionMsg(req_v, req_n, orderingVMsg.m, id, id, id, orderingVMsg.rcvtime+ Simulator.orderingDelta);
                ...
                if(Simulator.SHOWDETAILINFO){
                    orderedNum++;
                    System.out.println("排序结束,发送第"+orderedNum+"预准备消息"+messageDecisionMsg.toString()+"---"+messageDecisionMsg.m.rcvtime+ Simulator.orderingDelta);
    ...
    public void receiveMessageD(Message msg){
        if(isTimeOut) return;
        MessageDMsg orderingPMsg = (MessageDMsg)msg;
        Message req = orderingPMsg.m;
        int req_v = orderingPMsg.v;
        int req_n = orderingPMsg.n;
        int i = orderingPMsg.i;
...
//		    2. 将本OV消息发送到主节点消息广播
            ...
            addMessageToCache(msg,req_n);
            //生成当前orderingVote消息
            Message messageVoteMsg = new MessageVoteMsg(req_v, req_n, orderingPMsg.m, id, id, id, msg.rcvtime);
//		2. 将本OV消息发送到主节点消息广播
            Simulator.sendMsgToReplica(messageVoteMsg,id,getPriId(),sendTag);
        }
    }
