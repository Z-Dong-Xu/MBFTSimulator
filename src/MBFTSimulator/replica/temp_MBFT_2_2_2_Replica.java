...
/**
 * @ClassName: temp_MBFT_2_2_2_Replica
 * @Description:
 * @Author: Dongxu Zhu
 * @Create: 2023-02-07 20:58
 **/
public class temp_MBFT_2_2_2_Replica {
    //正在处理的状态位
    ...
    public temp_MBFT_2_2_2_Replica(int id, int[] netDlys, int[] netDlyToClis) {
        this.id = id;
...
//		2. 将本OV消息发送到主节点消息广播
    ...
    public void receiveOrderingV(Message msg){
        ...
        if(hasRequset(req_n)){
            ...
            //===========
//            //节点将所有已经收到的预准备消息构造成集合prePrepareMsgSet
//            ProcessingCache processingCache = msgCache.get(req_n);
//		    若预准备消息集合为空，则返回，否则继续
            if(ordered(req_n)){
                PrePrepareMsg prePrepareMsg = new PrePrepareMsg(req_v, req_n, orderingVMsg.m, id, id, id, orderingVMsg.rcvtime);
                Simulator.sendMsgToOthers(prePrepareMsg, id, sendTag);
                addMessageToCache(prePrepareMsg,req_n);
                if(Simulator.SHOWDETAILINFO){
                    orderedNum++;
                    System.out.println("排序结束,发送第"+orderedNum+"预准备消息"+prePrepareMsg.toString()+"---"+prePrepareMsg.m.rcvtime);
    ...
    public void receivePreprepare(Message msg) {
        ...
        Message req = prePrepareMsg.m;
        int req_v = prePrepareMsg.v;
        int req_n = prePrepareMsg.n;
        int i = prePrepareMsg.i;

        if(hasRequset(req_n)){
            addMessageToCache(msg,req_n);
            Message prepareMsg = new PrepareMsg(req_v, req_n, req, id, id, id, msg.rcvtime);
//		    1.将本prepare加入当前节点的msgCache中的prepare集合
            addMessageToCache(prepareMsg,req_n);
//		    2. 将本prepare消息广播
...
//		    2. 将本prepare消息广播
    ...
    public boolean prepared(int req_n) {
        ...
        if(cache.prePare_N >= 2 * Utils.getMaxTorelentNumber(Simulator.RN)) {
    ...
    public boolean commited(int req_n) {
        ...
        if(cache.prePare_N >= (2 * Utils.getMaxTorelentNumber(Simulator.RN)+1)  ) {
